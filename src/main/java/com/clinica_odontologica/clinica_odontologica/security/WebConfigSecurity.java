...
public class WebConfigSecurity {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(AbstractHttpConfigurer::disable) // Desactivado para esta aplicacion
				.authorizeHttpRequests((auth) -> auth.requestMatchers("/pacienteLista.html").permitAll()
						.requestMatchers("/odontologoLista.html").permitAll().requestMatchers("/turnoLista.html")
						.permitAll()
						.requestMatchers("/auth/register", "/login.html", "/css/**", "/js/**", "/register.html")
						.permitAll().requestMatchers("/auth/me").authenticated().anyRequest().authenticated())
